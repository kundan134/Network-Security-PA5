...
def SS( num, iConfidence ):
    ...
    if __gcd( a, num ) > 1:
        return False
    if not _jacobi( a, num ) % num == __modexp ( a, (num-1)//2, num ):
        return False
    return True
...
def _jacobi( a, n ):
    if a == 0:
        ...
        return _jacobi( a%n, n)
    elif a%2 == 0:
        return _jacobi(2, n)*_jacobi(a//2, n)
    else:
        if a % 4 == 3 and n%4 == 3:
            return -1 * _jacobi( n, a)
        else:
            return _jacobi(n, a )
...
def find_primitive_root( p ):
    ...
    while( 1 ):
        ...
        if not (__modexp( g, (p-1)//p1, p ) == 1):
            if not __modexp( g, (p-1)//p2, p ) == 1:
                return g
...
def encrypt(key, sPlaintext):
    ...
    for i in z:
        ...
        c = __modexp( key.g, y, key.p )
        d = (i*__modexp( key.h, y, key.p)) % key.p
...
def decrypt(key, cipher):
    ...
    for i in range(0, len(cipherArray), 2):
        ...
        s = __modexp( c, key.x, key.p )
        plain = (d*__modexp( s, key.p-2, key.p)) % key.p
...
if __name__=='__main__':
    ...
    cipher = encrypt(cipherKeys['publicKey'], "Message to be encrypted")
    print(cipher)
    plaintext=decrypt(cipherKeys['privateKey'], cipher)
    print(plaintext)
